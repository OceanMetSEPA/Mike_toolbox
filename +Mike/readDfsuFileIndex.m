function [dfsuData]=readDfsuFileIndex(dfsuFileName,varargin)
...
%
% 20180205 - This function is a modified version of MIKE.readDfsuFile, and
% seems to be faster, particularly when a limited number of indices are required.
% Rather than extracting all timesteps and then filtering by index,
% it extracts one timestep at a time, filtering as it goes
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% $Workfile:   readDfsuFileIndex.m  $
% $Revision:   1.1  $
% $Author:   Ted.Schlicke  $
% $Date:   Apr 05 2019 11:47:08  $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if nargin==0
    help MIKE.readDfsuFileIndex
    return
end
...
NOutputElements=double(dfsu.NumberOfElements); % NLayers*NMeshElements
NOutputNodes=double(dfsu.NumberOfNodes); % (NLayers+1)*NMeshNodes
NLayers = double(dfsu.NumberOfLayers); % 0 for 2d dfsu outpuf file
if NLayers==0
    NLayers=1;
...
NMeshElements=double(NOutputElements/NLayers);
NMeshNodes=NOutputNodes/(NLayers+1);
%NMeshNodes=double(dfsu.NumberOfNodes/(NLayers+1));
% Spatial information:
...
for parameterIndex=1:NParameters
    parameter=dfsu.ItemInfo.Item(parameterIndex-1);
    ...
    parameterInfo{parameterIndex}=struct('Name',parameterName,'UnitDescriptions',unitDescription,'UnitAbbreviation',unitAbbreviation);
end
...
parameterInfo=parameterInfo(ismember(dfsuParameterNames,dfsuParameterNames2Extract));
...
NtInDfsu = double(dfsu.NumberOfTimeSteps);
if options.verbose
...
modelDatenums=(t0+double(0:(NtInDfsu-1))*timeSep); % time steps
if options.verbose
...
if options.ts==0 % Default - extract everything (0 used for this purpose in D3D functions)
    ...
    timeIndices2Extract=find(modelDatenums>=options.t0 & modelDatenums<options.t1 & mod(0:(NtInDfsu-1),dk)==0);
else
    timeIndices2Extract=options.ts;
    if timeIndices2Extract==-1
        fprintf('Extracting final timestep %d\n',NtInDfsu)
        timeIndices2Extract=NtInDfsu;
    end
end
...
Nt2Extract=length(modelDatenums);
...
dfsuData=struct('Name',dfsuFileName,'DateTime',modelDatenums(:),'X',x(:),'Y',y(:),'Z',z,'ElementTable',elementTable,'MeshIndices',tri2d,'XMesh',xn2d,'YMesh',yn2d,'ZMesh',zn2d,'Index',indices2Extract,'ItemInfo',parameterInfo);

%% Load data  OLD WAY
% INVOLVED LOADING ALL TIMESTEPS THEN FILTERING LAYERS/INDICES
% FOR BIG FILES, THIS REQUIRES BIG MEMORY ALLOCATION
% if options.data
...
%         pin=stringFinder(dfsuParameterNames,p,'output','index');
%         if options.verbose
...
%% NEW (2018) way - load timesteps one at a time, filtering by index each time.
...
for parameterIndex=1:Np
    ...
    %    parameterData=arrayfun(@(i)double(dfsu.ReadItemTimeStep(pin,i-1).Data)',timeIndices2Extract,'Unif',0);
    % New way:
    ca=cell(Nt2Extract,1);
    for timeIndex=1:Nt2Extract;
        ithTimeIndex=timeIndices2Extract(timeIndex);
        parameterData=double(dfsu.ReadItemTimeStep(pin,ithTimeIndex-1).Data);
        % Check to see if we have one value per mesh centre. Most of our
        % parameters of interest (water level, speed, salinity etc) have
        % this
        if length(parameterData)==NOutputElements
            parameterData=reshape(parameterData,NLayers,[]);
            parameterData=parameterData(:,indices2Extract)';
            %        else % One value per mesh vertex. Don't do anything to this for now, but may want to in the future
        end
        ca{ithTimeIndex}=parameterData;
    end
    % Now reshape data for all timesteps
    parameterData=horzcat(ca{:});
    %     try
    %         parameterData=reshape(parameterData,[],NLayers,Nt2Extract);
    %     catch
    %         parameterData=reshape(parameterData,[],Nt2Extract);
    %     end
    ...
    if options.squeeze
        parameterData=squeeze(parameterData);
    end
    fn=itemFieldNames{parameterIndex};
    dfsuData.(fn)=parameterData;
end
